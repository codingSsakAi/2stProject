...
class MLRecommendationService:
            ...
            if created:
                profile, created = UserProfile.objects.get_or_create(
                    ...
                    defaults={
                        ...
                        "selected_insurance": np.random.choice(
    ...
    def _preprocess_data(self):
        ...
        try:
            ...
            # 특성 스케일링
            self.user_features = self.scaler.fit_transform(
                self.user_df[numeric_features]
            )
        ...
        try:
            ...
            if user_product_data:
                ...
                self.user_product_matrix = self.user_product_df.pivot_table(
                    index="user_id", columns="product", values="rating", fill_value=0
                )
            else:
    ...
    def get_content_based_recommendations(
        self, user_profile: Dict[str, Any], top_k: int = 5
    ) -> List[Dict[str, Any]]:
        """콘텐츠 기반 필터링 추천"""
        try:
    ...
    def get_hybrid_recommendations(
        self, user_id: int, user_profile: Dict[str, Any], top_k: int = 5
    ) -> List[Dict[str, Any]]:
        """하이브리드 추천 (협업 + 콘텐츠)"""
        try:
            # 협업 필터링 추천
        ...
        try:
            ...
            for other_user_id in self.user_product_matrix.index:
                if other_user_id != user_id:
                    other_vector = self.user_product_matrix.loc[
                        other_user_id
                    ].values.reshape(1, -1)
    ...
    def _calculate_content_score(
        self, user_profile: Dict[str, Any], product: PolicyDocument
    ) -> float:
        """콘텐츠 기반 점수 계산"""
        try:
            score = 0.0
            ...
            if age < 30:
                if (
                    "student" in product.title.lower()
                    or "young" in product.title.lower()
                ):
                    score += 2.0
            elif age > 50:
                if (
                    "senior" in product.title.lower()
                    or "mature" in product.title.lower()
                ):
                    score += 2.0
            ...
            if gender == 1:  # 남성
                ...
                if (
                    "female" in product.title.lower()
                    or "women" in product.title.lower()
                ):
                    score += 1.0
            ...
            if driving_exp < 3:
                if (
                    "new" in product.title.lower()
                    or "beginner" in product.title.lower()
                ):
                    score += 1.5
            elif driving_exp > 10:
                if (
                    "experienced" in product.title.lower()
                    or "premium" in product.title.lower()
                ):
                    score += 1.5
            ...
            if mileage > 15000:
                if (
                    "high" in product.title.lower()
                    or "extended" in product.title.lower()
                ):
                    score += 1.0
            elif mileage < 8000:
            ...
            if accidents > 0:
                if (
                    "comprehensive" in product.title.lower()
                    or "full" in product.title.lower()
                ):
                    score += 1.5
    ...
    def _combine_recommendations(
        self, collaborative_recs: List[Dict], content_recs: List[Dict]
    ) -> List[Dict]:
        """추천 결과 결합"""
        try:
            combined = {}
            ...
            for rec in content_recs:
                ...
                if product_name in combined:
                    ...
                    combined[product_name]["hybrid_score"] += (
                        rec["score"] * 0.4
                    )  # 가중치 0.4
                else:
                    combined[product_name] = {
    ...
    def generate_recommendations(
        self, user_id: int, user_profile: Dict[str, Any]
    ) -> Dict[str, Any]:
        """통합 추천 생성"""
        try:
            # 하이브리드 추천 생성
            recommendations = self.get_hybrid_recommendations(
                user_id, user_profile, top_k=10
            )
            ...
            recommendation_data = {
                ...
                "ml_scores": {
                    "collaborative_score": sum(
                        r.get("collaborative_score", 0) for r in recommendations
                    ),
                    "content_score": sum(
                        r.get("content_score", 0) for r in recommendations
                    ),
                    "hybrid_score": sum(
                        r.get("hybrid_score", 0) for r in recommendations
                    ),
                },
                    ...
                    similar_users.append(
                        {
                            ...
                            "driving_experience": int(
                                self.user_df.iloc[idx]["driving_experience"]
                            ),
    ...
    def get_recommendation_history(self, user_id: int) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            user = User.objects.get(id=user_id)
            recommendations = MLRecommendation.objects.filter(user=user).order_by(
                "-recommendation_date"
            )
    ...
    def get_recommendation_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            viewed_recommendations = MLRecommendation.objects.filter(
                is_viewed=True
            ).count()
            selected_recommendations = MLRecommendation.objects.filter(
                is_selected=True
            ).count()

            # 평균 만족도
            avg_satisfaction = (
                UserProfile.objects.aggregate(
            ...
            return {
                ...
                "view_rate": (
                    viewed_recommendations / total_recommendations
                    if total_recommendations > 0
                    else 0
                ),
                "selection_rate": (
                    selected_recommendations / total_recommendations
                    if total_recommendations > 0
                    else 0
                ),
