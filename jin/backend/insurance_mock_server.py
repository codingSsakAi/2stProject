...
class InsuranceMockServer:
    ...
    def calculate_premium(self, user_profile: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for company_name, company_info in self.insurance_companies.items():
            ...
            # 운전경력 할인 (최대 50% 할인)
            experience_discount = min(
                driving_experience * company_info["experience_bonus"], 0.5
            )
            ...
            quotes.append(
                {
                    ...
                    "special_discount": (
                        company_info["special_discount"]
                        if special_discount_applied
                        else None
                    ),
                    "discount_rate": f"{int(experience_discount * 100)}%",
                    "penalty_rate": (
                        f"{int(accident_penalty * 100)}%"
        ...
        return {
            ...
            "market_analysis": {
                ...
                "average_premium": int(
                    sum(q["annual_premium"] for q in quotes) / len(quotes)
                ),
                "price_difference": quotes[-1]["annual_premium"]
                - quotes[0]["annual_premium"],
    ...
    def _calculate_risk_level(self, user_profile: Dict[str, Any]) -> str:
        ...
        age_category = self.get_age_category(
            user_profile.get("birth_date", "1990-01-01")
        )
    ...
    def _get_partnership_benefits(self, company_name: str) -> List[str]:
        ...
        return partnerships.get(
            company_name, ["제휴카드 할인", "주유소 할인", "마트 할인"]
        )
